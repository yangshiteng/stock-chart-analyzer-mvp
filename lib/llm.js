...
function validateReasoningFormat(analysis, mode) {
  ...
  const missing = [];
  if (mode === "entry") {
    if (!reasoning.includes("current=")) missing.push("current=");
    if (!/candidates/i.test(reasoning)) missing.push("candidates");
    if (!reasoning.includes("mode=")) missing.push("mode=");
  } else if (mode === "exit" || mode === "first_exit") {
    if (!reasoning.includes("zone=")) missing.push("zone=");
  }
  if (missing.length > 0) {
    throw new Error(
...
const FUZZY_WORDS_PATTERN = /(\blooks like\b|\blooks (?:bullish|bearish|strong|weak)\b|\bfeels (?:strong|weak|like)\b|\bseems (?:like|to)\b|\bshould (?:rebound|hold|bounce|recover)\b|\bprobably\b|\blikely\b|\bmomentum (?:building|fading|strong|weak)\b|\bbullish\b|\bbearish\b)/i;
...
function validateAnchorCrossCheck(analysis) {
  const reasoning = `${analysis.reasoning || ""}`;
  const match = reasoning.match(/(?:chose|target)\s*=\s*([\d.]+)\s*@\s*([A-Za-z_][A-Za-z0-9_]*)/);
  if (!match) return;
