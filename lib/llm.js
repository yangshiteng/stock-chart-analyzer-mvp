...
function getFallbackText(language, key, value = "") {
  const en = {
    ...
    holdOnlyRisk: "If you want the assistant to consider sell-side actions later, turn that rule on explicitly. Until then, treat this as a hold-only setup.",
    loosenBuyingRules: "If you want the assistant to consider entry actions later, loosen the current buying constraints first.",
    ...
    sellDisabledReason: "Sell-side actions are disabled in your current rules. Treat this as a reference sell level only.",
  ...
  const zh = {
    ...
    holdOnlyRisk: "如果你以后希望助手考虑卖出类动作，请显式打开这个规则。在此之前，应把当前情形视为只能持有。",
    loosenBuyingRules: "如果你以后希望助手考虑新的入场动作，请先放宽当前的买入限制。",
    ...
    sellDisabledReason: "你当前禁用了卖出类动作。这一档仅作为限价卖出参考位，不作为立刻执行的卖单建议。",
