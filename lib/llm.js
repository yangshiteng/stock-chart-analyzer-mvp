...
function buildPendingOrderSummary(side, pendingOrders) {
  ...
  if (shares > 0 && price !== null && price !== undefined && price !== "") {
    return `${shares} shares at ${price}`;
  }

  return "none";
}

function getPendingOrderSnapshot(payload, side) {
  const isBuy = side === "buy";

  return {
    price: isBuy ? payload.pendingOrders?.limitBuyPrice : payload.pendingOrders?.limitSellPrice,
    shares: Number(isBuy ? payload.pendingOrders?.limitBuyShares ?? 0 : payload.pendingOrders?.limitSellShares ?? 0)
  };
}
...
function getOrderRiskStyle(payload, side) {
  const fallback = `${payload.rules?.riskStyle || "conservative"}`.trim();
  const value = side === "buy"
    ? `${payload.rules?.buyRiskStyle || fallback}`.trim()
    : `${payload.rules?.sellRiskStyle || fallback}`.trim();
...
function buildFallbackSellGuidance(analysis, payload, language = "en") {
  const sellSideAllowed = Boolean(payload.rules?.allowReducingPosition);
  const currentShares = Number(payload.positionContext?.currentShares ?? 0);
  const currentPrice = getCurrentPriceNumber(analysis);
...
function buildAnalysisPromptFromConfig(config, payload) {
  ...
  return [
    ...
    `Sell-side actions allowed: ${formatBoolean(payload.rules?.allowReducingPosition)}.`,
    `Selected buy risk style: ${formatPromptValue(payload.rules?.buyRiskStyle || payload.rules?.riskStyle)}.`,
    `Selected sell risk style: ${formatPromptValue(payload.rules?.sellRiskStyle || payload.rules?.riskStyle)}.`,
