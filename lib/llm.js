...
function buildLanguageOutputSection(language) {
  if (language === "zh") {
    return formatBulletSection("LANGUAGE_OUTPUT", [
      ...
      "Keep action and all schema keys exactly in English.",
...
function formatLastSignalAndOrderSection(lastSignal, pendingLimitOrder, mode) {
  ...
  if (pendingLimitOrder) {
    ...
    lines.push(
      ...
      `Snapshot at placement: entry ${pendingLimitOrder.limitPrice}, stop ${pendingLimitOrder.stopLossPrice || "unspecified"}, target ${pendingLimitOrder.targetPrice || "unspecified"}.`,
    ...
    lines.push(
      `Your previous round recommended: action=${lastSignal.action}, orderPrice=${lastSignal.orderPrice ?? "null"}, entry=${lastSignal.entryPrice || "?"}, stop=${lastSignal.stopLossPrice || "?"}, target=${lastSignal.targetPrice || "?"}.`,
