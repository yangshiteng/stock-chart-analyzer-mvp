...
function buildReviewJsonSchema(allowedActions) {
  return {
    type: "object",
    additionalProperties: false,
    properties: {
...
function buildPremarketDipPlanJsonSchema() {
  return {
    type: "object",
    additionalProperties: false,
    properties: {
      ...
      referenceClose: { type: "string" },
      discountPercent: { type: "string" },
      nearestSupport: { anyOf: [{ type: "string" }, { type: "null" }] },
      supportStrength: {
        anyOf: [
          { type: "string", enum: MARKET_CONTEXT_LEVEL_STRENGTHS },
          { type: "null" }
        ]
      },
      reasoning: { type: "string" }
...
function validateReviewLongSetupPrices(review) {
  const entry = parsePriceField(review.orderPrice, "orderPrice");
  const stop = parsePriceField(review.stopLossPrice, "stopLossPrice");
  const target = parsePriceField(review.targetPrice, "targetPrice");

  if (stop >= entry) {
    throw new Error("Model returned invalid review: stopLossPrice must be below orderPrice.");
  }

  if (target <= entry) {
    throw new Error("Model returned invalid review: targetPrice must be above orderPrice.");
  }

  const risk = entry - stop;
  const reward = target - entry;
  if (reward / risk < 1) {
    throw new Error("Model returned invalid review: target must offer at least 1:1 reward-to-risk.");
  }
}
...
function validateSignalReviewResult(review, mode = "entry", currentPriceForExit = null) {
  const normalizedMode = mode === "exit" || mode === "force_exit" ? mode : "entry";
  if (!review || typeof review !== "object") {
    throw new Error("Model returned an invalid review object.");
  }

  if (!["KEEP", "REVISE", "EXPLAIN_ONLY"].includes(review.reviewDecision)) {
    throw new Error("Model returned invalid reviewDecision.");
  }

  const allowedActions = getAllowedActions(normalizedMode);
  if (!allowedActions.includes(review.action)) {
    throw new Error(
      `Model returned review action=${review.action || "missing"}, but ${normalizedMode} mode only allows ${formatList(allowedActions)}.`
    );
  }

  if (!["low", "medium", "high"].includes(review.confidence)) {
    throw new Error("Model returned invalid review confidence.");
  }

  if (typeof review.explanation !== "string" || !review.explanation.trim()) {
    throw new Error("Model returned invalid review explanation.");
  }

  parseNullablePriceField(review.stopLossPrice, "stopLossPrice");
  parseNullablePriceField(review.targetPrice, "targetPrice");

  if (review.action === "BUY_LIMIT") {
    validateReviewLongSetupPrices(review);
  } else if (review.action === "SELL_LIMIT") {
    const orderPrice = parsePriceField(review.orderPrice, "orderPrice");
    const currentPrice = parseNullablePriceField(currentPriceForExit, "currentPrice");
    if (normalizedMode === "exit" && currentPrice !== null && orderPrice <= currentPrice) {
      throw new Error("Model returned invalid review SELL_LIMIT: orderPrice must be above currentPrice in exit mode.");
    }
  } else if (review.action === "WAIT" || review.action === "HOLD") {
    validateNoOrderPrice(review);
  } else if (review.action === "SELL_NOW") {
    validateNoOrderPrice(review);
  }
...
function validatePremarketDipPlanResult(plan, {
  symbol = null,
  referenceClose = null,
  discountPercent = PREMARKET_DIP_DISCOUNT_PERCENT
} = {}) {
  if (!plan || typeof plan !== "object") {
    throw new Error("Model returned an invalid premarket dip plan object.");
  }

  if (plan.action !== "BUY_LIMIT") {
    throw new Error("Premarket dip plan must return action=BUY_LIMIT.");
  }

  const expectedSymbol = normalizeExpectedSymbol(symbol);
  const returnedSymbol = normalizeExpectedSymbol(plan.symbol);
  if (expectedSymbol && returnedSymbol !== expectedSymbol) {
    throw new Error(`Premarket dip plan returned symbol=${returnedSymbol || "missing"}, expected ${expectedSymbol}.`);
  }

  const returnedClose = parsePriceField(plan.referenceClose, "referenceClose");
  const expectedClose = referenceClose === null || referenceClose === undefined || referenceClose === ""
    ? returnedClose
    : parsePriceField(referenceClose, "referenceClose");
  if (Math.abs(returnedClose - expectedClose) / expectedClose > 0.001) {
    throw new Error("Premarket dip plan returned a referenceClose that does not match the user's input.");
  }

  const orderPrice = parsePriceField(plan.orderPrice, "orderPrice");
  const stopLossPrice = parsePriceField(plan.stopLossPrice, "stopLossPrice");
  const targetPrice = parsePriceField(plan.targetPrice, "targetPrice");
  if (stopLossPrice >= orderPrice) {
    throw new Error("Premarket dip plan returned invalid prices: stopLossPrice must be below orderPrice.");
  }
  if (targetPrice <= orderPrice) {
    throw new Error("Premarket dip plan returned invalid prices: targetPrice must be above orderPrice.");
  }

  const risk = orderPrice - stopLossPrice;
  const reward = targetPrice - orderPrice;
  if (reward / risk < 1) {
    throw new Error("Premarket dip plan target must offer at least 1:1 reward-to-risk.");
  }

  // Hard guardrail against FOMO entries: this tool is for a deep premarket dip,
  // not buying a shallow -2% pullback just because the stock is hot.
  if (orderPrice > expectedClose * 0.95) {
    throw new Error("Premarket dip plan orderPrice is too close to yesterday's close; expected a conservative dip entry.");
  }

  if (!["low", "medium", "high"].includes(plan.confidence)) {
    throw new Error("Premarket dip plan returned invalid confidence.");
  }

  const discountText = `${plan.discountPercent || ""}`.trim();
  if (!discountText || !discountText.includes(`${discountPercent}`)) {
    throw new Error(`Premarket dip plan must use the fixed ${discountPercent}% dip threshold.`);
  }

  const nearestSupport = plan.nearestSupport === null || plan.nearestSupport === undefined || plan.nearestSupport === ""
    ? null
    : `${plan.nearestSupport}`.trim();
  if (nearestSupport !== null) {
    parsePriceField(nearestSupport, "nearestSupport");
  }

  const supportStrength = plan.supportStrength === null || plan.supportStrength === undefined || plan.supportStrength === ""
    ? null
    : plan.supportStrength;
  if (supportStrength !== null && !MARKET_CONTEXT_LEVEL_STRENGTHS.includes(supportStrength)) {
    throw new Error("Premarket dip plan returned invalid supportStrength.");
  }

  const reasoning = `${plan.reasoning || ""}`.trim();
  if (!reasoning) {
    throw new Error("Premarket dip plan returned empty reasoning.");
  }

  return {
    action: "BUY_LIMIT",
    symbol: expectedSymbol || returnedSymbol,
    orderPrice: `${plan.orderPrice}`.trim(),
    stopLossPrice: `${plan.stopLossPrice}`.trim(),
    targetPrice: `${plan.targetPrice}`.trim(),
    confidence: plan.confidence,
    referenceClose: `${plan.referenceClose}`.trim(),
...
function formatVirtualPositionLines(virtualPosition, sellStrategy = null) {
  ...
  if (sellStrategy) {
    lines.push(
      ...
      `User max-loss delta: ${sellStrategy.maxLossDelta}.`,
      `Max-loss trigger price: ${sellStrategy.maxLossPrice}.`
    );
  }
...
function formatOriginalSignalLines(analysis) {
  if (!analysis) {
    return ["No original signal was recorded."];
  }

  return [
    `action: ${analysis.action || "unknown"}`,
    `orderPrice: ${analysis.orderPrice ?? "null"}`,
    `entryPrice: ${analysis.entryPrice ?? "null"}`,
    `stopLossPrice: ${analysis.stopLossPrice ?? "null"}`,
    `targetPrice: ${analysis.targetPrice ?? "null"}`,
    `confidence: ${analysis.confidence || "unknown"}`,
    `currentPrice: ${analysis.currentPrice || "unknown"}`,
    `reasoning: ${analysis.reasoning || "not recorded"}`
  ];
}
...
function buildPremarketDipPlanPrompt(payload, language) {
  const symbol = normalizeExpectedSymbol(payload.symbol) || "UNKNOWN";
  const referenceClose = `${payload.referenceClose || ""}`.trim();
  const referenceDipPrice = calculatePremarketDipReferencePrice(
    referenceClose,
    PREMARKET_DIP_DISCOUNT_PERCENT
  );
  const marketContextSection = formatMarketContextSection(
    payload.marketContext,
    "Use this higher-timeframe context as the only map for selecting a conservative premarket dip limit."
  );
  const lang = language === "zh"
    ? "Write reasoning in natural Simplified Chinese, <=120 Chinese characters. Keep schema keys/action/confidence in English."
    : "Write reasoning in English, <=120 characters.";

  return [
    formatSection("ROLE", [
      "You are creating a premarket dip-buy plan for a US equity day-trading assistant."
    ]),
    formatSection("OBJECTIVE", [
      "Produce one conservative BUY_LIMIT plan for 04:00-09:30 ET only.",
      "This is a pre-open preparation plan, not a live 5-minute chart execution signal."
    ]),
    formatSection("PREMARKET_INPUTS", [
      `Symbol: ${symbol}.`,
      `User-entered yesterday close: ${referenceClose}.`,
      `Fixed defensive dip threshold: ${PREMARKET_DIP_DISCOUNT_PERCENT}%.`,
      `Reference dip price: ${referenceDipPrice}.`
    ]),
    marketContextSection,
    formatBulletSection("PLAN_RULES", [
      "Always return action=BUY_LIMIT.",
      `Start from the fixed ${PREMARKET_DIP_DISCOUNT_PERCENT}% dip reference price; do not make the entry materially shallower because of FOMO.`,
      "Prefer a nearby Market Context support below or near the reference dip price; strong support beats medium support, medium beats weak.",
      "If the nearest support is above the reference dip price, stay conservative: do not chase toward yesterday close.",
      "If no usable support exists near the reference dip price, use a conservative price around the reference dip price and lower confidence.",
      "Return exactly one executable orderPrice the user can place at the broker.",
      "stopLossPrice must be below orderPrice; targetPrice must be above orderPrice; reward-to-risk must be at least 1:1.",
      "Do not request additional chart images. Use only the text inputs and Market Context above."
    ]),
    formatBulletSection("OUTPUT_RULES", [
      lang,
      "Return strict JSON only.",
      "discountPercent must be \"10%\".",
      "nearestSupport is the chosen support price as a string, or null if no support is used.",
      "supportStrength is \"strong\", \"medium\", \"weak\", or null.",
      "Required fields: {\"action\":\"BUY_LIMIT\",\"symbol\":string,\"orderPrice\":string,\"stopLossPrice\":string,\"targetPrice\":string,\"confidence\":\"low\"|\"medium\"|\"high\",\"referenceClose\":string,\"discountPercent\":string,\"nearestSupport\":string|null,\"supportStrength\":\"strong\"|\"medium\"|\"weak\"|null,\"reasoning\":string}"
    ])
  ].join("\n\n");
}

function formatPendingLimitReviewLines(pendingLimitOrder) {
  if (!pendingLimitOrder) {
    return ["No pending broker limit order is currently tracked."];
  }

  const held = minutesSince(pendingLimitOrder.placedAt);
  const heldText = Number.isFinite(held) ? `${held} minute(s)` : "unknown age";

  return [
    `There is already a real pending broker order: ${pendingLimitOrder.action} at ${pendingLimitOrder.limitPrice}.`,
    `Order age: ${heldText}.`,
    `Stop-loss snapshot: ${pendingLimitOrder.stopLossPrice || "unspecified"}.`,
    `Target snapshot: ${pendingLimitOrder.targetPrice || "unspecified"}.`,
    `Confidence snapshot: ${pendingLimitOrder.confidence || "unknown"}.`,
    `Reasoning snapshot: ${pendingLimitOrder.reasoning || "not recorded"}.`
  ];
}

function buildSignalReviewPrompt(payload, language) {
  const mode = payload.mode === "exit" || payload.mode === "force_exit" ? payload.mode : "entry";
  const allowedActions = getAllowedActions(mode);
  const sanitizedUrl = sanitizeUrl(payload.pageUrl);
  const symbolHint = payload.symbolHint || guessSymbol(payload.pageTitle, sanitizedUrl || payload.pageUrl) || "unknown";
  const modeLabel = mode === "force_exit" ? "FORCE_EXIT" : mode === "exit" ? "EXIT" : "ENTRY";
  const lang = language === "zh"
    ? "Write explanation in natural Simplified Chinese, <=120 Chinese characters."
    : "Write explanation in English, <=120 characters.";
  const marketContextSection = formatMarketContextSection(payload.marketContext);

  const sections = [
    formatSection("ROLE", [
      "You are reviewing one already-generated intraday trading signal. This is a second-pass check, not a casual chat."
    ]),
    formatSection("OBJECTIVE", [
      "Decide whether the original signal should be kept, revised, or only explained after considering the user's challenge."
    ]),
    formatSection("SESSION_MODE", [
      `Mode: ${modeLabel}.`,
      `Allowed actions in this review: ${formatList(allowedActions)}.`
    ]),
    formatSection("POSITION_CONTEXT", formatVirtualPositionLines(payload.virtualPosition, payload.sellStrategy)),
    ...(marketContextSection ? [marketContextSection] : []),
    formatSection("CHART_CONTEXT", [
      `Page title: ${payload.pageTitle || "Unknown"}`,
      `Page URL: ${sanitizedUrl || "Unknown"}`,
      `Symbol hint: ${symbolHint}.`,
      "Use only what is visible in the screenshot plus the user's challenge text as a hypothesis to verify."
    ]),
    formatSection("ORIGINAL_SIGNAL", formatOriginalSignalLines(payload.originalAnalysis)),
    formatSection("PENDING_LIMIT_ORDER", formatPendingLimitReviewLines(payload.pendingLimitOrder)),
    formatSection("USER_CHALLENGE", [
      `${payload.userChallenge || ""}`
    ]),
    formatBulletSection("REVIEW_RULES", [
      "The user's challenge is not automatically true. Treat it as a hypothesis or risk flag, then verify against the screenshot.",
      "Do not flatter or blindly agree with the user. Keep the original signal if the chart still supports it.",
      "If the user supplies intraday context not visible in the screenshot (for example: stock is already +15% today), factor it into risk and confidence but say how it changes the decision.",
      "If revising to BUY_LIMIT or SELL_LIMIT, return one exact orderPrice the user can place now.",
      "In EXIT mode, use SELL_NOW for immediate exit and SELL_LIMIT only for a take-profit limit above current price.",
      "Do not revise to a defensive SELL_LIMIT at/below current price; that should be SELL_NOW with orderPrice=null.",
      "If a pending broker limit order exists, review whether that existing order should keep resting, be cancelled, or be cancelled and replaced at a revised price.",
      "When a pending broker order exists, do not imply the user can place a second order. If you revise the price, say in explanation that the user should cancel the existing order before placing the revised one.",
      "If no order should be placed now, return WAIT in entry mode or HOLD in exit mode with orderPrice=null.",
      "Do not affect future rounds unless the user explicitly accepts the review signal."
    ]),
    formatBulletSection("OUTPUT_RULES", [
      "reviewDecision=KEEP means the original action remains correct.",
      "reviewDecision=REVISE means the action or executable price changed.",
      "reviewDecision=EXPLAIN_ONLY means the question is answered but the user should not place a new order from this review.",
      "For BUY_LIMIT, stopLossPrice < orderPrice < targetPrice and reward-to-risk must be at least 1:1.",
      "For SELL_LIMIT in EXIT mode, orderPrice must be above current price.",
      "For WAIT, HOLD, and SELL_NOW, orderPrice must be null.",
      lang
    ]),
    formatSection("OUTPUT_FORMAT", [
      "Return strict JSON only.",
      "Required fields: {\"reviewDecision\": \"KEEP\" | \"REVISE\" | \"EXPLAIN_ONLY\", \"action\": allowed action, \"orderPrice\": string | null, \"stopLossPrice\": string | null, \"targetPrice\": string | null, \"confidence\": \"low\" | \"medium\" | \"high\", \"explanation\": string}"
    ])
  ];
...
export async function generatePremarketDipPlan(payload) {
  const settings = await getSettings();
  const language = getLanguage(settings.language);

  if (!settings.openaiApiKey) {
    throw new Error("Add your OpenAI API key in the popup before generating a premarket dip plan.");
  }

  const symbol = normalizeExpectedSymbol(payload.symbol);
  const prompt = buildPremarketDipPlanPrompt(payload, language);
  ...
  const rawText = await callOpenAi({
    ...
    model: settings.model || "gpt-5.4",
    userContent,
    schemaName: "premarket_dip_buy_plan",
    schema: buildPremarketDipPlanJsonSchema(),
    maxOutputTokens: PREMARKET_DIP_PLAN_MAX_OUTPUT_TOKENS
  });

  const parsed = parseJsonResponse(rawText, "Premarket dip plan");
  return validatePremarketDipPlanResult(parsed, {
    symbol,
...
export async function analyzeSignalReview(payload) {
  const settings = await getSettings();
  const language = getLanguage(settings.language);
  ...
  const mode = payload.mode === "exit" || payload.mode === "force_exit" ? payload.mode : "entry";
  const prompt = buildSignalReviewPrompt(payload, language);
  const userContent = [
    { type: "input_text", text: prompt },
    { type: "input_image", image_url: payload.imageDataUrl, detail: "high" }
  ];
  ...
  const rawText = await callOpenAi({
    ...
    model: settings.model || "gpt-5.4",
    userContent,
    schemaName: "stock_signal_review",
    schema: buildReviewJsonSchema(getAllowedActions(mode)),
    maxOutputTokens: REVIEW_MAX_OUTPUT_TOKENS
  });

  const parsed = parseJsonResponse(rawText, "Signal review");
  return validateSignalReviewResult(parsed, mode, payload.originalAnalysis?.currentPrice ?? null);
}
