...
function validateAnalysisResult(analysis, mode = "entry", buyStrategy = null) {
  ...
  if (analysis.action === "BUY_LIMIT") {
    ...
    // Enforce user's dip-buy discount: orderPrice must be <= currentPrice -
    // dipBuyDiscount. Compute the max from the AI's freshly-returned
    // currentPrice rather than a stale precomputed value — that way the
    // constraint always reflects the price the AI just saw, not a hint from
    // a previous round.
    if (buyStrategy && buyStrategy.dipBuyDiscount) {
      const orderPrice = parsePriceField(analysis.orderPrice, "orderPrice");
      const discount = Number(buyStrategy.dipBuyDiscount);
      if (Number.isFinite(discount) && discount > 0) {
        const maxAllowed = currentPrice - discount;
        if (orderPrice > maxAllowed) {
          throw new Error(
            `Model returned BUY_LIMIT orderPrice ${orderPrice.toFixed(2)} above the user's max allowed ${maxAllowed.toFixed(2)} (currentPrice ${currentPrice.toFixed(2)} - dipBuyDiscount ${discount.toFixed(2)}). The user is opting out of marketable limits; wait for a deeper dip or return WAIT.`
          );
        }
      }
    }
...
function formatBuyStrategySection(buyStrategy, mode) {
  // Only inject in entry mode — there's no BUY_LIMIT in exit / force_exit, so
  // the discount constraint is irrelevant. Skip silently when buyStrategy is
  // missing or has no discount.
  if (mode !== "entry" || !buyStrategy || !buyStrategy.dipBuyDiscount) {
    return null;
  }
  const lines = [
    `User-set dip-buy discount: $${buyStrategy.dipBuyDiscount}.`
  ];
  // Concrete max-price hint when we have a recent currentPrice. Round 1 won't
  // have it; AI must compute it from its own observed currentPrice.
  if (buyStrategy.currentPrice && buyStrategy.maxOrderPrice) {
    lines.push(
      `Reference current price (from previous round): $${buyStrategy.currentPrice}.`,
      `Maximum allowed BUY_LIMIT orderPrice ≈ $${buyStrategy.maxOrderPrice} (reference - discount). The actual gate uses your freshly-observed currentPrice this round.`
    );
  } else {
    lines.push(
      "First round — no previous-round price hint. Compute the gate yourself: max allowed orderPrice = your freshly-observed currentPrice - dipBuyDiscount."
    );
  }
  lines.push(
    "Any BUY_LIMIT you issue MUST have orderPrice <= (your observed currentPrice - dipBuyDiscount). The user is opting OUT of marketable limits; do NOT chase pullbacks at near-current price.",
    "If the chart shows a hot setup but the right entry is above this max, return WAIT with orderPrice=null — missing the trade is the user's accepted cost for avoiding chop-kill stop-outs.",
    "If you see a deeper, valid level at or below the max (real support / prior low / structural pullback), issue BUY_LIMIT at that level."
  );
  return formatBulletSection("BUY_STRATEGY", lines);
}
...
function buildAnalysisPromptFromConfig(config, payload, language) {
  ...
  const buyStrategySection = formatBuyStrategySection(payload.buyStrategy, mode);
  if (buyStrategySection) {
    sections.push(buyStrategySection);
  }
...
export async function analyzeChartCapture(payload) {
  ...
  for (let attempt = 1; attempt <= ANALYSIS_VALIDATION_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      return validateAnalysisResult(parsed, mode, payload.buyStrategy);
    } catch (error) {
