...
// The aggressive-evidence requirement is "≥2 concrete observable evidence
// items, each backed by a number". We previously enforced this by requiring
// the literal markers "(1)" AND "(2)" in the reasoning — but that rejected
// VALID evidence written in other shapes, e.g.
//   "mode=aggressive(距S1仅0.01,S1-S2差0.04)"  ← 2 real numeric items,
//                                                  comma-separated, no (1)/(2)
// which caused a real round to fail and auto-pause. The marker syntax was a
// proxy that was too literal.
//
// New rule: count numeric tokens in the EVIDENCE REGION (everything from the
// aggressive marker onward — which contains the evidence clause plus the
// chose/target price). Require ≥2 numbers there. This accepts any phrasing
// (Chinese commas, semicolons, "(1)/(2)", dashes) while still rejecting
// pure-prose aggressive calls like "mode=aggressive (strong setup)" (which
// has only the 1 chose-price number after the marker). The fuzzy-word ban
// remains the primary guard against confident-sounding but evidence-free text.
const AGGRESSIVE_MARKER_PATTERN = /(mode\s*=\s*aggressive|flow\s*=\s*push-rebound|target\s*=\s*aggressive|trend\s*=\s*strong)/i;

function validateAggressiveEvidence(analysis, mode) {
  if (!isAggressiveChoice(analysis, mode)) return;
  const reasoning = `${analysis.reasoning || ""}`;

  // Locate the aggressive marker; evidence is everything from there onward.
  // (anchorSource=aggressive_recovery can trigger isAggressiveChoice without a
  // textual marker — in that case scan the whole reasoning.)
  const markerMatch = reasoning.match(AGGRESSIVE_MARKER_PATTERN);
  const evidenceRegion = markerMatch
    ? reasoning.slice(markerMatch.index)
    : reasoning;

  // Count numeric tokens (integers or decimals) in the evidence region.
  const numbers = evidenceRegion.match(/\d+(?:\.\d+)?/g) || [];
  if (numbers.length < 2) {
    throw new Error(
      `Aggressive choice in ${mode} mode requires ≥2 numeric evidence references in reasoning ` +
        `(e.g., "(1) 3-bar lows 30.4->30.3->30.2; (2) spread 0.9" or "距S1仅0.01,S1-S2差0.04"). ` +
        `Found ${numbers.length} number(s) in the evidence region. Got: "${reasoning}"`
    );
  }

  // Fuzzy-word blacklist (primary guard against evidence-free confident text)
...
function formatLastSignalAndOrderSection(lastSignal, pendingLimitOrder, mode) {
  ...
  if (pendingLimitOrder) {
    ...
    lines.push(
      ...
      `- MANDATORY: re-rank the candidate pool (S1/S2 supports below current, or R1/R2 above) FROM SCRATCH on the CURRENT chart every round, BEFORE looking at the resting order. The resting order must NOT anchor your fresh analysis. Then compare: if fresh S1 (or R1) is a DIFFERENT level than the resting order's, or the same anchor's value has moved, you MUST re-emit the new number — that is a realign/switch, NOT chasing.`,
      `- Distinguish CHASING from a STRUCTURAL UPDATE: chasing = nudging the SAME anchor a tick because price wiggled (don't). Structural update = the nearest level is now a DIFFERENT one than before (e.g., price climbed and EMA20 rose above the old static anchor, so S1 switched from prior_low to EMA20), OR the same anchor drifted meaningfully (EMA20 27.50 → 27.80). When S1/S2 genuinely changed, re-emitting is REQUIRED, not optional. A position that sits unfilled for many rounds while price trends away is a RED FLAG that you are stale-repeating instead of re-ranking.`
    );
    if (lastSignal) {
      lines.push(
...
function formatMarketContextSection(
  ...
  const lines = [
    ...
    "Key levels (role is dynamic — anything below current price acts as support, anything above acts as resistance; if price crosses a level, its role inverts). A [×N confluence] tag means N source levels stack at that price — a more loaded / stronger zone, worth weighing when choosing between two candidates:"
  ];
  ...
  if (keyLevels.length === 0) {
    ...
    for (const level of keyLevels) {
      const zone = level.zoneLow && level.zoneHigh
        ...
        : "";
      const confluence = Number(level.confluence) >= 2 ? ` [×${Math.floor(level.confluence)} confluence]` : "";
      lines.push(
        `- ${level.type || "pivot"} (${level.timeframe || "?"}) ${level.label || ""}: ${level.price || "?"}${zone}${confluence}. ${level.reason || ""}`.trim()
      );
    }
  }
...
function buildMarketContextScanPrompt(payload, language) {
  ...
  if (timeframe === "daily") {
    ...
    // 15m — fills the gap between 1H (5-20 days) and the live 5-min window
    // (~1-2 days). Captures intraday consolidation shelves / reaction levels
    // from the last few days that neither other timeframe shows clearly.
    setupLines = [
      "The user should be on a TradingView 15-minute candlestick chart.",
      "Visible history should cover roughly 3-5 trading days.",
      "Candles, volume, and EMA 20 / 50 / 100 / 200 should be visible.",
      "Focus on recent intraday structure: consolidation shelves (price bands where several candles based), reaction highs / lows, and clear intraday pivots from the last few days.",
      "VWAP is optional on 15m. If visible, use it only as secondary context.",
      "Visible-range High / Low labels should be hidden; do not treat them as true support or resistance."
    ];
  }
  ...
  return [
    ...
    formatBulletSection("OUTPUT_RULES", [
      ...
      lang,
      "Required fields: {\"timeframe\": \"daily\" | \"1h\" | \"15m\", \"regime\": \"uptrend\" | \"range\" | \"downtrend\", \"keyLevels\": KeyLevel[], \"riskNotes\": string}",
...
export async function analyzeMarketContextScan(payload) {
  ...
  const rawText = await callOpenAi({
    ...
    model: settings.model || "gpt-5.4",
    userContent,
    schemaName: `market_context_${timeframe === "1h" ? "hourly" : timeframe === "15m" ? "minute15" : "daily"}_scan`,
