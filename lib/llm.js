...
// Limit-only design (real-trade testing showed market orders cause too much
// slippage / bad fills). The only place SELL_NOW survives is FORCE_EXIT,
// where the day-trading rule "must be flat by close" outweighs the slippage
// concern. Normal entry / exit go through limit orders only; the user places
// the limit at the broker, marks it placed, and the "Limit filled" button
// promotes it to a real position when the order fills.
const ALLOWED_ACTIONS = ["BUY_LIMIT", "SELL_NOW", "SELL_LIMIT", "HOLD", "WAIT"];
const ENTRY_MODE_ACTIONS = ["BUY_LIMIT", "WAIT"];
const EXIT_MODE_ACTIONS = ["SELL_LIMIT", "HOLD"];
...
function validateLongSetupPrices(analysis) {
  const entry = parsePriceField(analysis.entryPrice, "entryPrice");
  ...
  if (stop >= entry) {
    throw new Error("Model returned invalid long setup: stopLossPrice must be below entryPrice.");
  }

  if (target <= entry) {
    throw new Error("Model returned invalid long setup: targetPrice must be above entryPrice.");
  }
...
function validateAnalysisResult(analysis, mode = "entry") {
  ...
  for (const field of ["entryPrice", "stopLossPrice", "targetPrice", "currentPrice"]) {
    parsePriceField(analysis[field], field);
  }
...
function formatLastSignalAndOrderSection(lastSignal, pendingLimitOrder, mode) {
  ...
  if (pendingLimitOrder) {
    ...
    lines.push(
      ...
      "- CRITICAL: if the previous trigger condition is now satisfied on this chart, you MAY upgrade the action within the current allowed vocabulary — e.g. WAIT can become BUY_LIMIT in entry mode; HOLD can become SELL_LIMIT in exit mode. State in reasoning which condition was met.",
...
function buildAnalysisPromptFromConfig(config, payload, language) {
  ...
  // Long-term context: structural read of a daily/weekly chart, generated once
  // at session start and frozen for the whole session. Placed BEFORE
  // last-signal continuity so the model reads structural anchors before
  // deciding whether to repeat the previous round's call.
