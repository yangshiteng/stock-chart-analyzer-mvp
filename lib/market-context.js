...
const VALID_TIMEFRAMES = new Set(["daily", "1h", "15m"]);

// Representative-priority when merging confluent levels across timeframes:
// daily > 1h > 15m (longer timeframe = wider relevance = preferred
// representative price/label for the merged candidate).
function timeframeRank(timeframe) {
  if (timeframe === "daily") return 0;
  if (timeframe === "1h") return 1;
  return 2; // 15m (and any unknown) ranks last
}
...
export function normalizeMarketContext(value) {
  ...
  // COMPLETE requires all THREE scans (daily + 1h + 15m) plus a built summary.
  // An older stored context that only has daily + hourly (pre-15m) recomputes
  // to HOURLY_SCANNED here on read, which correctly prompts the user to scan
  // 15m before COMPLETE.
  if (normalized.summary && normalized.dailyScan && normalized.hourlyScan && normalized.minute15Scan) {
    normalized.status = MARKET_CONTEXT_STATUS.COMPLETE;
  } else if (normalized.dailyScan && normalized.hourlyScan) {
    normalized.status = MARKET_CONTEXT_STATUS.HOURLY_SCANNED;
...
export function mergeMarketContextScans({ dailyScan, hourlyScan, minute15Scan, symbol, tradingDay }) {
  if (!dailyScan || !hourlyScan || !minute15Scan) {
    throw new Error("Daily, 1H, and 15m market context scans are all required.");
  }

  // Regime is a higher-timeframe read — resolved from daily + 1h only. The 15m
  // scan's regime is too short-term / noisy to define the overall regime, so we
  // take its KEY LEVELS but ignore its regime classification.
  ...
  const keyLevels = mergeConfluentLevels([
    ...
    ...(Array.isArray(minute15Scan.keyLevels) ? minute15Scan.keyLevels : [])
  ]);
  const notes = [
    dailyScan.riskNotes ? `Daily: ${dailyScan.riskNotes}` : "",
    hourlyScan.riskNotes ? `1H: ${hourlyScan.riskNotes}` : "",
    minute15Scan.riskNotes ? `15m: ${minute15Scan.riskNotes}` : ""
  ].filter(Boolean).join(" ");
