export const ANALYSIS_RESPONSE_SCHEMA = '{"action": "BUY_NOW" | "BUY_LIMIT" | "SELL_NOW" | "SELL_LIMIT" | "WAIT", "entryPrice": string, "stopLossPrice": string, "targetPrice": string, "triggerCondition": string, "confidence": "low" | "medium" | "high", "reasoning": string, "symbol": string | null, "currentPrice": string}';
...
export const EXECUTION_PROMPT_CONFIG = {
  ...
  actionRules: [
    ...
    "Use SELL_NOW when price is at a level you would immediately exit an open long (momentum breaking down at resistance, stop-loss touched).",
    ...
    "Use WAIT only when no level on the chart justifies action right now. Even on WAIT, you must still return hypothetical entry / stop / target prices describing the setup you would take IF it triggered.",
    "Never overuse WAIT when the chart shows a credible actionable setup."
  ],
  executionRules: [
