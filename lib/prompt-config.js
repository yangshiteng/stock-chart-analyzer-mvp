export const ANALYSIS_RESPONSE_SCHEMA = '{"action": "BUY_LIMIT" | "SELL_NOW" | "SELL_LIMIT" | "HOLD" | "WAIT", "orderPrice": string | null, "entryPrice": string | null, "stopLossPrice": string | null, "targetPrice": string | null, "confidence": "low" | "medium" | "high", "reasoning": string, "symbol": string | null, "currentPrice": string}';
...
export const EXECUTION_PROMPT_CONFIG = {
  ...
  exitModeRules: [
    ...
    "If current price is at or below the max-loss trigger price, or below the recorded stop-loss, return SELL_NOW unless the screenshot shows an immediate, credible reclaim already in progress.",
  ...
  executionRules: [
    ...
    "confidence=high only when ALL of: (a) clear chart structure, (b) readable numeric levels, (c) an executable orderPrice or a clear no-action HOLD/WAIT decision, (d) volume and VWAP (when visible) agree with the direction of the trade. If volume or VWAP contradicts the setup, cap at 'medium' or lower.",
