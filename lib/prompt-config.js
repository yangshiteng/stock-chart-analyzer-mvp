...
export const EXECUTION_PROMPT_CONFIG = {
  ...
  firstExitModeRules: [
    "SESSION_MODE=FIRST_EXIT: the user just clicked 'Limit filled' — a BUY_LIMIT in the broker just filled. This is a one-shot analysis to set the dual stops AND the initial SELL_LIMIT.",
    "Allowed actions: SELL_LIMIT (normal case) or SELL_NOW (rare: a catastrophic gap-down has already pushed price below where any reasonable hard stop would sit at fill moment).",
    "REQUIRED output (in normal SELL_LIMIT case):",
    "  - stopLossPrice (soft stop) = the nearest key level strictly BELOW currentPrice. This is the 'thesis weakening' line — when crossed, position enters Recovery zone but does NOT auto-sell.",
    "  - hardStopPrice (hard stop) = the nearest key level strictly BELOW stopLossPrice. This is the 'thesis dead' line — when crossed, position immediately SELL_NOW. Hard stop MUST be strictly below soft stop.",
    ...
    "If no key level exists below currentPrice (rare, e.g. price at all-time low), use a conservative chart-based estimate for stopLossPrice and anchorSource='conservative_estimate'. Note in reasoning.",
    "If softStop and hardStop would coincide (sparse key levels below), set hardStop to a conservative chart-based estimate further down. Note in reasoning.",
    "If currentPrice is already at or below where any reasonable hardStop would sit (catastrophic gap-down), return action=SELL_NOW with anchorSource='stop_broken' and skip the SELL_LIMIT. The position is doomed; exit now.",
  ...
  executionRules: [
    ...
    "first_exit MUST emit stopLossPrice + hardStopPrice with hardStopPrice strictly < stopLossPrice strictly < currentPrice (except SELL_NOW gap-down case).",
