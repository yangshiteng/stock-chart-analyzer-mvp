...
export const EXECUTION_PROMPT_CONFIG = {
  ...
  objective: "Review the 5-minute chart screenshot. Mechanically determine which zone currentPrice is in, then either (a) apply mechanical rules (hard-exit SELL_NOW; force_exit SELL_NOW; first_exit per-zone defaults) or (b) make a constrained AI subjective conservative/aggressive judgment (buy mode S1/S2 supports; healthy zone trend=normal R1 / trend=strong R2 resistances; observation zone 走止盈/走解套; caution zone 保守/激进). There is NO WAIT or HOLD — always emit a price (or SELL_NOW). The user decides at the broker whether to actually place each order.",
  chartFocusAreas: [
  ...
  entryModeRules: [
    ...
    "STEP 1 — Build the candidate pool FRESH every round from the CURRENT chart (re-read all level values; do NOT carry over last round's pool or let a resting limit order anchor you): collect ALL key levels strictly BELOW currentPrice from STATIC (MARKET_CONTEXT pivots/gaps/prior_high/prior_low), DYNAMIC (current values of EMA 20/50/100/200 + VWAP — these DRIFT each round), and INTRADAY STATIC. For INTRADAY STATIC you must ACTIVELY SCAN the recent candle structure (not just read labels): include any consolidation shelf (≥2-3 candles based at a price band), recent reaction low, or intraday_pivot that sits below current. COMMON MISTAKE TO AVOID: defaulting to EMA20 as the nearest support when a clearer intraday shelf/reaction-low sits between current and EMA20 — that nearer intraday level is the real S1. SECOND COMMON MISTAKE: when price has trended up since the order was placed, the EMAs have risen too — S1 must track the NEW nearest support, not stay frozen at the old placed price. DO NOT use fixed_soft_stop / fixed_hard_stop / aggressive_recovery in entry mode — those are post-fill anchors.",
    "STEP 1.5 — CONFLUENCE MERGE (do this BEFORE labeling S1/S2): collapse levels that sit within ~0.2% of each other (≈ a few cents on a sub-$50 stock) into ONE candidate, regardless of source type. A static pivot at 27.00 + EMA200 at 27.01 + an intraday shelf at 27.02 are the SAME price zone — they must occupy ONE slot, not eat S1+S2+S3. For the merged candidate: price = the static level's price if one is in the cluster, else the cluster's middle; anchorSource = the most structural source present (static > intraday > dynamic). A zone where MORE levels stack is a STRONGER level — note it in reasoning (e.g. 'S1=27.00 [3-way: pivot+EMA200+shelf]') and treat higher confluence as a legitimate factor favoring that level in the S1-vs-S2 judgment.",
    "STEP 2 — Sort the merged candidates by distance from currentPrice ascending. Label S1 = nearest support, S2 = 2nd nearest support, S3+ = FORBIDDEN (do NOT use as target). (Buy-side levels are SUPPORTS below current, hence S1/S2 — not R1/R2, which are resistances on the sell side.)",
  ...
  exitModeRules: [
    ...
    "CONFLUENCE MERGE (resistances above current, same as the buy side): collapse levels within ~0.2% of each other (any mix of static / EMA / VWAP / intraday / fixed_*) into ONE candidate before labeling R1/R2 — they're the same zone and must occupy one slot, not crowd it. Representative: price = static level's price if present, else cluster middle; anchorSource = most structural present (static > intraday > dynamic > fixed_*). Note confluence in reasoning (e.g. 'R1=28.20 [2-way: prior_high+EMA50]'); a higher-confluence resistance is a stronger ceiling — weigh it in the trend=normal/strong (R1 vs R2) judgment.",
