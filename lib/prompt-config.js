...
export const EXECUTION_PROMPT_CONFIG = {
  ...
  objective: "Review the 5-minute chart screenshot. Mechanically determine which zone currentPrice is in, then either (a) apply mechanical rules (healthy zone R1/R2 by K-line trend, hard-exit SELL_NOW) or (b) make a constrained AI subjective judgment (buy mode R1/R2 = conservative/aggressive; observation zone 走止盈/走解套; caution zone 保守/激进). There is NO WAIT or HOLD — always emit a price (or SELL_NOW). The user decides at the broker whether to actually place each order.",
  chartFocusAreas: [
    "Key levels — STATIC (from MARKET_CONTEXT: pivot, gap, prior_high, prior_low), DYNAMIC (current values of EMA 20 / EMA 50 / EMA 100 / EMA 200 and VWAP visible on the 5-minute chart), and INTRADAY STATIC (today's HOD/LOD, opening-range high/low ORH/ORL, and any intraday_pivot formed by repeated rejection/reclaim during the session). All levels have equal weight — no strength tier.",
  ...
  // ----- ENTRY MODE -------------------------------------------------------
  // BUY mode with R1/R2 + AI subjective judgment (conservative/aggressive).
  ...
  entryModeRules: [
    ...
    "STEP 1 — Build the candidate pool: collect ALL key levels strictly BELOW currentPrice from STATIC (MARKET_CONTEXT pivots/gaps/prior_high/prior_low), DYNAMIC (current values of EMA 20/50/100/200 + VWAP), and INTRADAY STATIC (today's HOD/LOD, opening-range, intraday_pivot). DO NOT use fixed_soft_stop / fixed_hard_stop / aggressive_recovery in entry mode — those are post-fill anchors.",
    "STEP 2 — Sort by distance from currentPrice ascending. Label R1 = nearest support, R2 = 2nd nearest, R3+ = FORBIDDEN (do NOT use as target). If multiple candidates are within 1-2 ticks of each other, merge into one R item using the confluence priority (static > dynamic; anchorSource = the static one if present).",
    "STEP 3 — AI subjective judgment in 'mode = conservative | aggressive' (see EVIDENCE CHECKLIST + CONSTRAINTS below). Conservative = R1 (default; expect a small pullback). Aggressive = R2 (deeper pullback target with better cost basis but lower fill probability).",
    ...
    "STEP 5 — Output ALWAYS returns BUY_LIMIT. There is no WAIT. Even if R1 is far below current price, return BUY_LIMIT at R1 — the user decides at the broker whether to place it.",
    "EVIDENCE CHECKLIST for mode judgment (7 dimensions). When weighing 保守 vs 激进: (1) K-line shape: recent 3-5 bars direction, rising/falling lows/highs, green/red distribution; (2) EMA relationship: current vs EMA20/50, EMA stack (bull/bear/tangled), EMA slope; (3) VWAP: current vs VWAP, recent reclaim/reject; (4) Volume: expansion/contraction, down-bar volume vs bounce-bar volume; (5) Distance to R1: R1 too close to current = aggressive R2 has more value; R1 far = waiting for R1 already needs patience; (6) R1 vs R2 spread: spread small = aggressive ROI small; spread large (≥ R1-distance × 1.5) = aggressive ROI significant; (7) Market Context: large-cap regime, sector direction, overall risk appetite.",
    "DIRECTION: opposite of sell-side observation zone — bearish/weak evidence pushes toward aggressive R2 (deep pullback likely). Bullish/strong evidence stays conservative R1 (shallow pullback expected).",
    "CONSTRAINT 1 (HARD RULE): default = conservative R1. If evidence is mixed, neutral, or bullish, you MUST return mode=conservative. 'Uncertain = conservative' is not negotiable. This prevents AI from packaging weak signals as 'deep pullback likely' via reasoning fluency.",
    "CONSTRAINT 2: mode=aggressive requires ≥2 specific OBSERVABLE EVIDENCE items in reasoning, each with a number or position reference. Bare adjectives like 'weak', 'momentum down', 'looks bearish' (without numbers) are forbidden. Compliant: 'evidence: (1) 3-bar lower highs 30.40→30.30→30.20 red bars; (2) R1-R2 spread 0.90 = R1-dist 0.40 × 2.25'. Non-compliant: 'evidence: (1) looks weak; (2) momentum bearish'.",
    ...
    "BOUNDARY — R1 too close to current (within 1-2 ticks, almost at-the-money): treat as natural aggressive signal — waiting for R1 ≈ market order, while R2 offers a real pullback target. Note in reasoning.",
    "BOUNDARY — only R1 exists (no R2 in pool): mode must be conservative (cannot pick R2 that doesn't exist).",
    ...
    "REASONING FORCED FORMAT (≤120 chars): MUST include `current=X; candidates=[R1=A@anchor1, R2=B@anchor2]; mode=conservative|aggressive (依据); chose=Y@<anchor>; placement=...; continuity=...`. The <anchor> in chose= must exactly equal the anchorSource field (cross-check)."
  ],
  // ----- FIRST_EXIT MODE --------------------------------------------------
  ...
  firstExitModeRules: [
    ...
    "  - **Healthy zone** (currentPrice > entryPrice): SELL_LIMIT @ R1 = entry-price-above first un-broken key level. Trend forced to 'normal' (no R2 / no strong-trend judgment in first-exit). Continues the buy thesis.",
    "  - **Observation zone** (softStop < currentPrice ≤ entryPrice): SELL_LIMIT @ R1 (same as healthy — continues the buy thesis; the small drift below entry post-fill is the noise you expected before the bounce). DO NOT force 走解套 here — that would sell-out on fresh-fill noise.",
    "  - **Caution zone** (hardStop < currentPrice ≤ softStop): SELL_LIMIT @ fixed_soft_stop (or nearer if an intraday candidate sits between current and softStop). Forced conservative mode — thesis already weakened by breaking softStop, no subjective upgrade.",
    ...
    "anchorSource: same enum as entry mode + 'stop_broken' (for the SELL_NOW case). NEVER fixed_* (this analysis is writing softStop/hardStop, can't refer to them yet). NEVER aggressive_recovery (no subjective upgrade in first-exit).",
  ...
  exitModeRules: [
    ...
    "**HEALTHY zone — MECHANICAL R1/R2 selection (no subjective judgment)**: build candidate pool of all key levels above entry; filter out those already broken (≤ currentPrice); R1 = nearest above current, R2 = 2nd nearest. R3+ FORBIDDEN. Default trend=normal → SELL_LIMIT @ R1. Strong-trend trigger (mechanical): past 3 consecutive 5-min K-lines are all green AND closes are strictly rising → SELL_LIMIT @ R2. Strong but R2 not available → fall back to R1, note 'no R2 available'.",
    "7-DIMENSION EVIDENCE CHECKLIST for caution/observation subjective judgment: (1) K-line shape (3-5 bar direction, rising/falling lows/highs, green/red); (2) EMA relationship (current vs EMA20/50, stack, slope); (3) VWAP (current vs VWAP, recent reclaim/reject); (4) Volume (expansion/contraction, bounce-vs-down ratio); (5) Distance to softStop (observation: closer = more recovery; caution: closer = more bullish, approaching reclaim); (6) Distance to hardStop (caution-specific: safety cushion, far = more aggressive room); (7) Market Context.",
    "DIRECTION: bullish/strong rebound evidence → upgrade to aggressive (observation: 走止盈 R1; caution: 激进 entry+$0.05). Default保守 when evidence is unclear/conflicting/bearish.",
    "CONSTRAINT 1 (HARD RULE): default = 保守/走解套. If evidence is mixed, neutral, or bearish, you MUST stay at the default. 'Uncertain = conservative' is not negotiable.",
    "CONSTRAINT 2: aggressive choice (观察区走止盈 OR 警戒区激进) requires ≥2 specific OBSERVABLE EVIDENCE items in reasoning, each with a number or position reference. Bare adjectives forbidden.",
    "CONSTRAINT 3: short-term vs medium-term conflict → default保守. Aggressive needs multi-dimensional agreement.",
  ...
  executionRules: [
    ...
    "reasoning must be ≤120 characters and follow the per-mode FORCED FORMAT (see entryModeRules / exitModeRules / firstExitModeRules / forceExitRules)."
  ],
  languageRules: [
    "reasoning must be short and concrete; ≤120 characters; cite the anchor by name (e.g. 'EMA20', 'prior_low', 'fixed_soft_stop', 'aggressive_recovery'); use forced format per mode.",
