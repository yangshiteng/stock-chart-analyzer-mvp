...
2. **区间内的具体目标选择有的机械、有的主观**:
   - **健康区 / 必须离场区**: 完全机械(健康区按 R1/R2 + K 线判据;必须离场区直接 SELL_NOW)
   - **观察区 / 警戒区**: 允许 AI 主观综合判断(带证据约束),详见 pillar 3
...
3. **观察区和警戒区允许 AI 主观综合判断**:健康区和必须离场区是纯机械规则(可审计、可回测)。观察区和警戒区是浮亏中、信号最丰富的两个状态——机械规则会丢失 K 线/EMA/VWAP/量能/Market Context 多个有用维度。所以这两个区允许 AI 做主观综合判断,但**都带三条相同的结构约束**:
   - 不确定 = 默认保守(观察区默认走解套 / 警戒区默认 softStop)
   - 激进选项必须列 ≥2 条数字证据
   - validator 拒绝模糊词
   
   两个区的判断是镜像设计:
   - **观察区**: 在「走止盈(R1) / 走解套(≤entry+0.05)」二选一
   - **警戒区**: 在「保守 target(softStop) / 激进 target(entry+0.05)」二选一
   
   这是有意识的局部试验,实测效果差会回退到机械规则(已有 v18 fallback)。
...
健康区是浮盈状态,**承受得起**让 SELL_LIMIT 挂远一点等更大利润——大不了没成交、下一轮重评。所以允许 AI 在每轮根据 K 线形态判断走势强弱:

| 走势强弱 | 触发条件(K 线形态硬条件) | Target |
|---|---|---|
| **普通**(默认) | 默认;或无明显持续上行迹象 | `R1`——入场价上方第一个未被突破的关键点位 |
| **强劲** | 过去 3 根 5 分钟 K 线**连续阳线** **且** 收盘价**strictly 连续抬高** | `R2`——入场价上方第二个未被突破的关键点位 |

**为什么只允许 R1 / R2,不允许 R3+**:强劲也有边界,避免贪心。R3+ 距离入场太远,成交概率剧降,长期 EV 不见得正。

**判据为什么只用 K 线形态**:同观察区设计原则。"连续阳线 + 收盘连续抬高"是图上一眼能数的事实;"势能强弱""动量好不好"这类主观判据不可靠,容易过度乐观。
...
观察区是**两个允许 AI 做主观综合判断的区间之一**(另一个是警戒区,设计对称;健康区和必须离场区是机械规则)。这里允许 AI 充分使用 reasoning 能力,综合多项观察证据,在「走止盈」和「走解套」之间二选一。
...
警戒区是 4 个区里**第二个允许 AI 做主观综合判断的区间**(第一个是观察区)。机械单 target 会丢失信号——比如量能突然放大 + EMA 反弹,本可以挂高一档争取近平本,机械规则却挂在 softStop 提早落袋。
...
**场景 A(健康区 R1↔R2)**: K 线形态在"强劲触发"边界附近反复(美股最小 tick = $0.01):
- bar 序列 `... 28.10, 28.11, 28.12`(每根抬 1 tick,strictly 抬高)→ 强劲触发 → R2
- 下一轮新 bar 序列 `... 28.11, 28.12, 28.12`(bar3 持平于 bar2)→ 退化为普通 → R1
- 再下一轮 `... 28.12, 28.12, 28.13` → 仍然不构成 strictly 连续抬高(中间一对持平)→ 普通 → R1
- 再下一轮 `... 28.12, 28.13, 28.14` → strictly 抬高 → 又回 R2
