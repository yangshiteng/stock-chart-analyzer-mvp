...
- **UI**: (a) optional textarea in the initial start-monitoring form; (b) a "Background Notes" card that appears while RUNNING/PAUSED with the current notes + Edit/Save/Cancel flow so the user can refine context mid-session (e.g. add "halted on news" as it happens). Char counter shows `N/500`. Notes pre-fill on restart from `lastMonitoringProfile`.
- **Handler**: `update-user-context` message → `updateUserContext(payload)`. Validates length ≤ 500, requires `STATUS.RUNNING`, patches `monitoringProfile.userContext` (and `lastMonitoringProfile` mirror). Edits are rejected while PAUSED — explicit choice: a paused session is read-only until the user Continues, consistent with mark-bought/mark-sold guards.
- **i18n**: 11 new keys (en + zh): `backgroundNotesTitle`, `backgroundNotesCopy`, `backgroundNotesPlaceholder`, `backgroundNotesCharLimit`, `backgroundNotesEmpty`, `editNotesButton`, `saveNotesButton`, `cancelNotesButton`, `backgroundNotesTooLong`, `backgroundNotesNotRunning`, `couldNotSaveNotes`.
