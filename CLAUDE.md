...
- **Design note**: the "user parameter → prompt explains → schema validates" pattern is still good for **truly user-specific preferences** (sell-side quickProfitDelta / maxLossDelta capture personal risk tolerance, which the AI cannot know). It's the wrong pattern for **AI execution judgment** like "how aggressive should this entry be" — that's exactly the AI's job, and parameterizing it just lets the user fight the model.
