...
Completed (v19 strategy unification + robustness — STATE_VERSION 19):

**Trigger** — a cascade of real-trade observations over several days that the v18 design, while structurally sound, had under-specified its candidate-pool, made the buy-side mechanical while the sell浮亏区 was AI-subjective (asymmetric), and used too-strict validator markers that auto-paused valid signals. This iteration unifies the design (all "二选一" decisions use the same subjective framework), broadens the candidate pool, and hardens the runtime against silent failures.

**Five pillars (each a deliberate, separately-confirmed design decision)**:

1. **Symmetric "保守/激进" subjective framework across ALL four two-choice decisions**: buy mode S1/S2 (renamed from R1/R2 — supports, not resistances), healthy zone R1/R2, observation 走止盈/走解套, caution 保守/激进. All four use the identical 7-dimension evidence checklist + 3 structural constraints (default conservative is hard rule; aggressive requires ≥2 numeric evidence; validator rejects fuzzy words). The only purely mechanical paths remaining are: hard-exit SELL_NOW, force_exit SELL_NOW, and first_exit per-zone defaults. Previously healthy-zone R1/R2 was a mechanical 3-green-bar trigger; v19 moved it under the subjective framework for full symmetry. K-line shape is now one of the 7 dimensions, not the sole trigger.

2. **Candidate pool expanded with intraday static levels + fixed stops as independent candidates**: v17 had STATIC (Market Context: pivot/gap/prior_high/prior_low) + DYNAMIC (EMA/VWAP). v19 adds INTRADAY STATIC (intraday_high/low, opening_range_high/low, intraday_pivot — visually identified from candle structure, NOT labeled) and persistent FIXED STOPS (fixed_soft_stop = virtualPosition.stopLossPrice, fixed_hard_stop = hardStopPrice — independent candidates from the now-drifted dynamic anchors that originally defined them). Reasoning: the EMA value an anchor was named after at first-exit time drifts away; the original number stays fixed and remains a meaningful structural level (the "softStop=27.00 was originally EMA50, which is now at 26.80" example). Both compete as equal candidates each round.

3. **Aggressive_recovery enum + zone-aware continuity (double safeguard)**: caution-zone激进 target uses `anchorSource = "aggressive_recovery"` (NEW enum), distinct from `conservative_estimate` (the passive fallback). Both target the same price (entry+$0.05) but represent semantically different decisions — aggressive_recovery is an ACTIVE upgrade decision; conservative_estimate is a no-anchor-available fallback. Mixing them would have caused continuity rules to misjudge cross-zone transitions as "repeat signal". Three-way continuity rules now compare `(zone, anchorSource, orderPrice)` tuples; `aggressive_recovery` is validator-locked to caution zone only.

4. **Reasoning forced format with per-mode markers + validator cross-checks**: every action emits structured reasoning that includes `current=` + `candidates=[S1=..@..,S2=..@..]` + `mode=conservative|aggressive` (entry) OR `zone=<name>` + per-zone sub-judgment (`trend=normal|strong` healthy / `flow=push-rebound|recovery` observation / `target=conservative|aggressive` caution / `next deep support=` hard-exit) + `chose|target=<price>@<anchor>` cross-check. The validator: (a) checks required markers are present (case-insensitive), (b) cross-checks the chose=/target= clause's price ±$0.05 matches `orderPrice` field, and anchor exactly matches `anchorSource` field, (c) counts numeric tokens in the evidence region (from the aggressive marker onward), requires ≥2 for aggressive choices — accepts ANY phrasing ("(1)/(2)", commas, Chinese, dashes), (d) bans fuzzy words (looks bullish, feels strong, momentum, etc.) without numeric backing, and (e) enforces per-mode anchorSource enum scope (fixed_* only after first_exit; aggressive_recovery only in caution zone; entry mode never sees post-fill anchors). 

5. **first_exit special cases corrected**: (a) per-zone defaults are MECHANICAL (no AI subjective at first-exit — there's no post-entry market behavior to evaluate yet); (b) healthy zone first-exit forces trend=normal → R1, and explicitly filters R1 to "above currentPrice" not just "above entryPrice" (matters when manual-holding position is in profit and entry<current; the level R1 would otherwise fall below current and validate-reject); (c) caution zone first-exit uses the SOFT-STOP price as target but with anchorSource = the underlying level name (e.g. EMA50), NEVER `fixed_soft_stop` (this analysis is WRITING the stop, can't reference a value it's just about to commit). Prompt previously contradicted itself ("target = fixed_soft_stop" but also "NEVER fixed_*"), causing deterministic validation failure for any manual-holding declared in caution zone.

**Robustness fixes (real-trade auto-pause / silent-hang bugs)**:

- **fetch() timeout via AbortController (90s ceiling)** in `callOpenAiOnce`. Pre-v19 the fetch had no timeout — a stalled OpenAI connection would make fetch wait forever, the MV3 service worker eventually got evicted, the side-panel's sendMessage never resolved, and the user saw a button stuck on "正在开始监控..." with no error. Timeout converts that into a retryable AbortError → callOpenAi's retry loop → clear error after ~90s. Also defensively marks bare network rejections as retryable.
- **ANALYSIS_MAX_OUTPUT_TOKENS bumped 1200 → 2000**. first_exit emits the longest output (forced reasoning + 3 stop fields + low-effort reasoning tokens). Complex charts could exceed 1200 → status=incomplete(max_output_tokens) → retryable loop → perceived hang.
- **Reasoning length cap 120 → 240 chars** in 4 prompt locations. The v19 forced formats run ~180-230 chars (especially entry/exit with ≥2 numbered evidence). The old "120 chars" cap forced the AI to drop required markers to fit → validateReasoningFormat failed → retry-loop slow. Added explicit "never drop a required marker to save space".
- **False-reject fix on aggressive evidence**: the old `validateAggressiveEvidence` required literal `(1)` AND `(2)` markers. Real AI output `mode=aggressive(距S1仅0.01,S1-S2差0.04)` — two valid numeric evidence items in Chinese comma format — was rejected → auto-pause. Rewrote to count numeric tokens in the evidence region (≥2 required) — accepts any phrasing while still rejecting pure-prose aggressive calls. Trade-off: digits inside labels like "S1"/"S2" can artificially boost the count, allowing contrived "S1 vs S2" prose evidence to pass. Accepted as a soft weakness — fuzzy-word ban catches the realistic failure mode, and tightening risks reintroducing false-rejects.

**Anti-EMA-default bias (label-anchoring fix)**:

LLM systematically defaulted to the labeled EMA values printed in the chart header (zero-cost to read) and skipped clearer, nearer intraday consolidation shelves / reaction lows (require visual candle analysis — harder). This mis-ranked the candidate pool — the intraday level should be S1 / R1, EMA should be S2 / R2 — so even the conservative default landed on the wrong (farther) level. Fix: broadened the INTRADAY STATIC definition from narrow "intraday_pivot formed by repeated rejection/reclaim" to include consolidation shelves (≥2-3 candles paused at a price band) and recent reaction low/high. Added explicit anti-default language in `chartFocusAreas`, `entryModeRules` STEP 1, `exitModeRules` (CANDIDATE POOL note), and a new `chartGuardrails` entry against "skipping a nearer intraday structural level for a farther labeled EMA/VWAP".

**Stale BUY_LIMIT fix (anti-anchoring)**: continuity prompt previously biased the model toward repeating the resting order when price ticked. One line — "only adjust when CHART STRUCTURE moved, not when price ticked" — was meant to prevent tick-chasing but over-suppressed legitimate realigns. When price climbs through EMAs, the nearest support (S1) genuinely becomes a different/higher level — that IS a structural change — but the model was reading it as "just ticking" and held the stale number for ~30 min. Fix: mandatory "re-rank candidate pool from scratch every round BEFORE looking at the resting order; resting order must not anchor the analysis", plus an explicit chasing-vs-structural-update distinction.

**UI**:
- **Sub-judgment pill** on the recommendation card's signal-banner — `extractSubJudgment` regex extracts `mode=` (entry) / `flow=` / `target=` / `trend=` markers from reasoning and `anchorSource === "aggressive_recovery"` as a deterministic caution-aggressive marker. Renders as a chip-style pill (保守/激进/普通/强劲/走止盈/走解套) so the user can see WHY this action without reading the reasoning paragraph. 8 new i18n keys (en + zh).
- **Zone labels renamed**: `zoneTakeProfit → zoneHealthy`, `zoneRecovery → zoneCaution`, new `zoneObservation`, retained `zoneHardExit / zoneStopsNotSet`. `computeZoneLabel` updated to 4-zone classification. Zone labels are render-time computed from current/entry/softStop/hardStop — never stored — so the rename is i18n-only with no migration.

**Files touched**:
- `lib/constants.js`: STATE_VERSION 18 → 19
- `lib/llm.js`: anchorSource enum extension (intraday_*, fixed_*, aggressive_recovery), per-mode allowed-anchors map, 4 mode-aware schema builders, validateAnalysisResult orchestrator, validateAnchorSource per-mode enum check, validateFirstExitResult entry-based stops + above-current rule, validateExitResult + validateAggressiveRecoveryZone (caution-only), validateReasoningFormat (case-insensitive marker presence), validateAggressiveEvidence (number-count + fuzzy-ban), validateAnchorCrossCheck (price ±$0.05 + anchor exact match, accepts R1=/S2=/aggressive= prefix variants), AbortController 90s timeout, ANALYSIS_MAX_OUTPUT_TOKENS=2000.
- `lib/prompt-config.js`: objective; chartFocusAreas (intraday levels broadened); chartGuardrails (anti-EMA-default + fuzzy-word ban); actionRules; entryModeRules (R1/R2→S1/S2 + AI subjective + 7-dim + 3 constraints + active-scan + boundaries); firstExitModeRules (per-zone mechanical defaults + ABSOLUTE-ABOVE-CURRENT rule + no fixed_* + reasoning forced format); exitModeRules (4-zone state machine + healthy subjective + observation/caution AI subjective + 7-dim + 3 constraints + INTRADAY STRUCTURE note); reasoning length 120→240 + "never drop markers".
- `lib/storage.js`: v19 migration hook (no-op + detailed comment explaining why no state mutation needed).
- `lib/i18n.js`: zone label renames (en + zh), 8 new pill labels (flowPushRebound/Recovery, targetConservative/Aggressive, trendNormal/Strong, entryModeConservative/Aggressive), softStop/hardStop/zone labels.
- `sidepanel.js`: computeZoneLabel 4-zone, extractSubJudgment/getSubJudgmentLabelKey/renderSubJudgmentPill, signal-banner pill insertion.
- `sidepanel.css`: `.sub-judgment-pill` (aggressive/conservative tone variants).
- `BUY_STRATEGY.md` + `SELL_STRATEGY.md`: full v19 rewrites with cross-referenced symmetric structure (each two-choice decision documents 7-dim + 3 constraints + monitoring metrics + fallback-to-mechanical option).
- Tests: 144 → 173.

**Pattern recognition / lessons that future iterations should remember**:

1. **Validator strictness is a UX risk**. Every "must use literal X format" check is a potential auto-pause hazard. Prefer counting semantic content (numbers, fuzzy words) over enforcing exact syntax. The pre-v19 "(1) AND (2)" check caused real auto-pauses on valid evidence written in Chinese commas. The fix (count numbers) has a known soft weakness (label-attached digits) but trades correctly — false-rejects (user-blocking) hurt much more than false-passes (caught downstream by other safeguards).

2. **Symmetric design pays maintenance dividends**. Once 3 of 4 zones used AI subjective with the 7-dim checklist + 3 constraints, moving the 4th (healthy) into the same framework was small (a few prompt lines + one `isAggressiveChoice` branch addition + test fixture updates). Tests, validators, UI, and the user mental model all transfer.

3. **MV3 + fetch without timeout = silent permanent hang**. Service worker eviction during long-running requests leaves the message port in limbo, the promise never resolves, and there's no error to surface. ALWAYS attach AbortController timeouts to fetch in MV3 background contexts. The previous "stuck on loading" reports for first-exit were entirely caused by this — no validation issue, just an unbounded fetch.

4. **LLM label-anchoring is a real cognitive bias**. Models default to numerically-labeled values (EMA20=27.79 shown in chart header) over visual structural levels that require analysis (a consolidation shelf at 28.00). This isn't laziness — it's the model minimizing inference work when both are presented as "valid options". Counteract with explicit anti-default language in the prompt AND broaden the candidate categories so the visual-only ones don't feel optional.

5. **Each "creation path" needs full feature coverage**. v18 had this lesson via the manual-holding bypass. v19 had it again via fresh-fill noise vs gap-down holding — first_exit must produce different defaults per zone because the same buy thesis means different things at different price positions. Both creation paths (markBought / confirmMarketContextAndStart→holding) go through the same `runFirstExitForPosition` helper since v18.

6. **Real-trade testing catches everything unit tests miss**. The fetch timeout, the first-exit caution contradiction, the false-reject auto-pause, the AI's label-anchoring bias — all surfaced in real use, none would have shown up in unit tests. Unit tests verify the VALIDATOR is correct; real trades verify the SYSTEM works end-to-end with the live model.
